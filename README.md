...
- `.github/workflows/publish.yml` installs enabled templates and filters through the Quarto CLI, renders the repo-provided examples or projects, writes a run summary, assembles PDFs into `assets/pdfs/*.pdf`, generates `index.qmd`, and publishes the site on pushes to `main`.
